    ...
    try {
      ...
      for (const project of submods) {
        if (statSync(project).isFile()) {
          ...
          const targetPackage = outdatedPackages.filter(p => p.name === packageToUpdate)
          core.info(`destinated Package: ${targetPackage[0].name} ${targetPackage[0].current}`)
          const DepWithVersion = `${targetPackage[0].name} ${targetPackage[0].current}`
          g.addVertex(targetPackage[0].current)
          g.addVertex(project)
          g.addEdge(project, targetPackage[0].current)
          core.endGroup()
        }
      }
      ...
      if (e instanceof Error) {
        ...
        core.setFailed("Some unknown error occurred, please see logs")
      }
    }
  }
  execute()